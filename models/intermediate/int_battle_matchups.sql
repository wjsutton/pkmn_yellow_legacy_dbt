...
trainer_pokemon_with_moves AS (
    ...
    SELECT
        ...
        pkmn_id,
    FROM TIL_PORTFOLIO_PROJECTS.dbt_wsutton_pkmn_dev.int_trainer_roster
    UNPIVOT (move FOR move_slot IN (move_1, move_2, move_3, move_4)) AS unpvt
    INNER JOIN TIL_PORTFOLIO_PROJECTS.dbt_wsutton_pkmn_dev.stg_moves_stats AS ms ON unpvt.move = ms.move
    LEFT JOIN
        TIL_PORTFOLIO_PROJECTS.dbt_wsutton_pkmn_dev.stg_moves_phys_spec AS physpec
        ON ms.type = physpec.type
    WHERE
