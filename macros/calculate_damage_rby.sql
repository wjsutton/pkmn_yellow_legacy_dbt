...
CASE 
    ...
    ELSE 
        (FLOOR(((LEAST(997,(FLOOR(FLOOR((2 * FLOOR({{ attacker_level }} / 5) + 2) * GREATEST(1,COALESCE({{ attacker_stat }},1)) * {{ move_power }}::double) / GREATEST(1,COALESCE({{ defender_stat }},1)))) / 50) + 2) * {{ stab }} * {{ move_type_effectiveness }}) / 2))
