-- team_verify  (MCP tool)
-- INPUT  : team[], badge, keep_pikachu, rival, legendary
-- OUTPUT : one summary row (team_quality, coverage) + one row per swappable slot
--          showing the best replacement and the quality it would add.
--          Use it to find which slots are weak / redundant and how to upgrade them.
--
-- Swap logic: for each member M, compute the team's best answer per opponent WITHOUT
-- M, then the best candidate C to put in M's place = max over C of
-- sum_opponents( max(best_without_M, win_value(C)) ). Local single-slot search.
...
candidates AS (
    ...
    WHERE player_pokemon NOT IN ({{ team_sql }})
      {% if no_legends %}AND player_pokemon NOT IN ('Moltres','Articuno','Zapdos','Mewtwo','Mew'){% endif %}
),
